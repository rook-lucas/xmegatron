// stats.js — File-based persistent stats
// Pair base: 63 | QR base: 17
...
export const BASE = { pair: 63, qr: 17 };
...
export function incrementPair(phone, sessionId) {
    cache.pair++;
    ...
    save(cache);
    console.log(`📊 Pair #${BASE.pair + cache.pair} — ${phone}`);
}

export function incrementQR(phone, sessionId) {
    cache.qr++;
    cache.history = cache.history || [];
    cache.history.unshift({
        type: 'qr',
    ...
    if (cache.history.length > 500) cache.history = cache.history.slice(0, 500);
    save(cache);
    console.log(`📊 QR #${BASE.qr + cache.qr} — ${phone || 'unknown'}`);
}
...
export function getStats() {
    return {
        ...
        history: (cache.history || []).slice(0, 30).map(h => ({
